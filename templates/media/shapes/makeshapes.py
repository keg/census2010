...
def readShapefile( type, filename ):
	...
	for feature in features:
		...
		info = feature['info']
		#print info
		name = info['NAME10'].decode( 'cp850' ).encode( 'utf-8' )
		name = re.sub( '\x00', '', name )
		statenum = info['STATEFP10']
		statenum = '8'
...
def keysplit( key ):
	...
	return type, { 'fips': '08',	'abbr': 'CO',	'name': 'Colorado',	}, name
...
def writeColoradoCounties( places, path ):
	p = {}
	for k in places:
		if places[k] != None:
			p[k] = places[k]
...
def generateColoradoCounties( detail, path ):
	print 'generateColoradoCounties'
	writeColoradoCounties( getPlaces(detail), path )
...
def getPlaces( detail ):
	...
	shapefile, places = readShapefile( 'county', 'shapefiles/county/tl_2010_08_county10.shp' )
	#shapefile, places = readShapefile( 'county', 'shapefiles/county/cntybnd2001.shp' )
	for key, place in places.iteritems():
...
generateColoradoCounties( 80, 'state' )
